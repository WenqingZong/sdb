...
template <class T> sdb::byte128 widen(const sdb::register_info& info, T t) {
    using namespace sdb;
    /*
    Like regular if, but runs at compile time. If we used a regular if
    statement, then wouldn't compile because some of the static_cast would be
    invalid. Using if constexpr statements allows for certain types of invalid
    code inside branches whose conditions aren't met.
    */
...
void sdb::registers::flush() {
    ...
    for (auto i = 0; i < 8; i++) {
        if (i == 4 or i == 5) {
            // Because DR4 and DR5 are obsolete aliases for DR6 and DR7.
            continue;
        }
