...
void execute_cfi_instruction(
    ...
    auto text_section_start = *elf.get_section_start_address(".text");
    auto plt_start =
        elf.get_section_start_address(".got.plt").value_or(sdb::file_addr{});
    ...
    if (primary_opcode) {
        ...
        case DW_CFA_def_cfa:
            ctx.cfa_rule =
                cfa_register_rule{static_cast<std::uint32_t>(cur.uleb128()),
...
sdb::registers execute_unwind_rules(unwind_context& ctx,
...
sdb::virt_addr read_frame_base_result(const sdb::dwarf_expression::result& loc,
    ...
    if (auto addr_res =
            std::get_if<sdb::dwarf_expression::address_result>(simple_loc)) {
        return addr_res->address;
    }
    sdb::error::send("Unsupported frame base location");
}

void scopes_at_address_in_die(const sdb::die& die, sdb::file_addr address,
...
sdb::die::children_range::iterator::operator++() {
    ...
    // 'this' is a pointer, and *this is dereference of the pointer, its
...
sdb::registers sdb::call_frame_information::unwind(const sdb::process& proc,
                                                   file_addr pc,
...
sdb::dwarf_expression::eval(const sdb::process& proc, const registers& regs,
                            bool push_cfa) const {
    ...
    while (!cur.finished()) {
        ...
        case DW_OP_bregx: {
            auto reg_val =
                regs.read(sdb::register_info_by_dwarf(cur.uleb128()));
            stack.push_back(std::get<std::uint64_t>(reg_val) + cur.sleb128());
            break;
        }
        case DW_OP_fbreg: {
            auto offset = cur.sleb128();
            auto fb_loc = func.value()[DW_AT_frame_base].as_evaluated_location(
...
sdb::dwarf_expression::result
sdb::location_list::eval(const sdb::process& proc,
...
sdb::dwarf_expression::result sdb::attr::as_evaluated_location(
...
std::optional<sdb::die> sdb::dwarf::find_local_variable(std::string name,
                                                        file_addr pc) const {
...
std::optional<sdb::die>
sdb::dwarf::get_member_function_definition(const sdb::die& declaration) const {
    index();
