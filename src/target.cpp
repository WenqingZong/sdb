...
void setup_arguments(sdb::target& target, sdb::die func,
                     std::vector<sdb::typed_data> args, sdb::registers& regs,
    ...
    std::array<sdb::register_id, 8> sse_regs = {
        sdb::register_id::xmm0, sdb::register_id::xmm1, sdb::register_id::xmm2,
        sdb::register_id::xmm3, sdb::register_id::xmm4, sdb::register_id::xmm5,
        sdb::register_id::xmm6, sdb::register_id::xmm7};
    ...
    for (auto i = 0; i < params.size(); ++i) {
        ...
        if (param.is_reference_type()) {
            if (args[i].address()) {
                args[i] = sdb::typed_data{sdb::to_byte_vec(*args[i].address()),
                                          sdb::builtin_type::integer};
            } else {
                rsp -= args[i].value_type().byte_size();
                rsp &= ~(args[i].value_type().alignment() - 1);
                target.get_process().write_memory(sdb::virt_addr{rsp},
                                                  args[i].data());
                args[i] = sdb::typed_data{sdb::to_byte_vec(rsp),
                                          sdb::builtin_type::integer};
            }
        }
    }
    ...
    for (auto& [arg, size] : stack_args) {
        target.get_process().write_memory(sdb::virt_addr{start_pos},
                                          arg.data());
        start_pos += size;
    }
...
sdb::typed_data read_return_value(sdb::target& target, sdb::die func,
                                  sdb::virt_addr return_slot,
                                  sdb::registers& regs) {
    auto ret_type = func[DW_AT_type].as_type();
    ...
    if (ret_classes[0] == sdb::parameter_class::memory) {
        ...
        return {sdb::typed_data{std::move(value), func[DW_AT_type].as_type(),
                                return_slot}};
    }
    ...
    for (auto ret_class : ret_classes) {
        if (ret_class == sdb::parameter_class::integer) {
            ...
            auto reg =
                used_sse ? sdb::register_id::xmm1 : sdb::register_id::xmm0;
            used_sse = true;
...
std::filesystem::path dump_vdso(const sdb::process& proc,
...
sdb::typed_data parse_argument(sdb::target& target, pid_t tid,
                               std::string_view arg) {
    if (arg.empty()) {
    ...
    if (arg.size() > 2 and arg[0] == '"' and arg[arg.size() - 1] == '"') {
        ...
        if (arg.find(".") != std::string::npos) {
            ...
            return {sdb::to_byte_vec(*value),
                    sdb::builtin_type::floating_point};
        } else {
...
std::vector<sdb::typed_data>
collect_arguments(sdb::target& target, pid_t tid, std::string_view arg_string,
...
sdb::die resolve_overload(const std::vector<sdb::die>& funcs,
    ...
    for (auto& func : funcs) {
        ...
        if (matches) {
            if (matching_func)
                sdb::error::send("Ambiguous function call");
            matching_func = func;
        }
    }
    if (!matching_func)
        sdb::error::send("No matching function");
    return *matching_func;
}

std::optional<sdb::typed_data>
inferior_call_from_dwarf(sdb::target& target, sdb::die func,
                         const std::vector<sdb::typed_data>& args,
...
sdb::file_addr
sdb::target::get_pc_file_address(std::optional<pid_t> otid) const {
    ...
    for (auto entry : dynamic_entries) {
        if (entry.d_tag == DT_DEBUG) {
            dynamic_linker_rendezvous_address_ =
                sdb::virt_addr{entry.d_un.d_ptr};
...
sdb::target::get_line_entries_by_line(std::filesystem::path path,
...
void sdb::target::notify_thread_lifecycle_event(const stop_reason& reason) {
    auto tid = reason.tid;
...
std::vector<std::byte>
sdb::target::read_location_data(const dwarf_expression::result& loc,
                                std::size_t size,
                                std::optional<pid_t> otid) const {
    auto tid = otid.value_or(process_->current_thread());
    if (auto simple_loc =
            std::get_if<sdb::dwarf_expression::simple_location>(&loc)) {
        if (auto reg_loc = std::get_if<sdb::dwarf_expression::register_result>(
                simple_loc)) {
            ...
            return {begin, begin + size};
                   ...
                   std::get_if<sdb::dwarf_expression::pieces_result>(&loc)) {
...
sdb::virt_addr sdb::target::inferior_malloc(std::size_t size) {
    ...
    auto malloc_func =
        std::find_if(malloc_funcs.begin(), malloc_funcs.end(),
                     [](auto& sym) { return sym.second->st_value != 0; });
...
sdb::target::evaluate_expression(std::string_view expr,
                                 std::optional<pid_t> otid) {
    auto tid = otid.value_or(process_->current_thread());
    auto pc = get_pc_file_address(tid);
    ...
    if (ret) {
        ...
        return evaluate_expression_result{std::move(*ret),
...
const sdb::typed_data& sdb::target::get_expression_result(std::size_t i) const {
    ...
    auto new_data =
        process_->read_memory(*res.address(), res.value_type().byte_size());
