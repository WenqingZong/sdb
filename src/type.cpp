...
sdb::parameter_class merge_parameter_classes(sdb::parameter_class lhs,
                                             sdb::parameter_class rhs) {
    using namespace sdb;
    if (lhs == rhs)
        return lhs;
    if (lhs == parameter_class::no_class)
        return rhs;
    if (rhs == parameter_class::no_class)
        return lhs;
...
void classify_class_field(const sdb::type& type, const sdb::die& field,
...
std::array<sdb::parameter_class, 2> classify_class_type(const sdb::type& type) {
    ...
    std::array<sdb::parameter_class, 2> classes = {
        sdb::parameter_class::no_class, sdb::parameter_class::no_class};
    if (type.get_die().abbrev_entry()->tag == DW_TAG_array_type) {
        ...
        if (type.byte_size() > 8 and
            classes[1] == sdb::parameter_class::no_class) {
            classes[1] = classes[0];
        }
    } else {
...
bool is_copy_or_move_constructor(const sdb::type& class_type,
                                 const sdb::die& func) {
...
std::size_t sdb::type::compute_byte_size() const {
    if (!is_from_dwarf()) {
        ...
        case builtin_type::integer:
            return 8;
        case builtin_type::floating_point:
            return 8;
...
sdb::typed_data
sdb::typed_data::read_member(const sdb::process& proc,
...
sdb::typed_data sdb::typed_data::index(const sdb::process& proc,
...
std::array<sdb::parameter_class, 2> sdb::type::get_parameter_classes() const {
    std::array<parameter_class, 2> classes = {parameter_class::no_class,
    ...
    if (tag == DW_TAG_base_type and stripped.byte_size() <= 8) {
        ...
        case DW_ATE_unsigned_char:
            ...
            break;
        default:
            sdb::error::send("Unimplemented base type encoding");
        }
...
bool sdb::type::is_reference_type() const {
    ...
    auto stripped = strip_cv_typedef().get_die();
    auto tag = stripped.abbrev_entry()->tag;
    return tag == DW_TAG_rvalue_reference_type;
}
