...
void set_ptrace_options(pid_t pid) {
    if (ptrace(PTRACE_SETOPTIONS, pid, nullptr,
        ...
        sdb::error::send_errno(
            "Failed to set TRACESYSGOOD and TRACECLONE option");
    }
}
...
sdb::stop_reason sdb::process::wait_on_signal(pid_t to_await) {
    int wait_status;
    // To wait on both processes and threads.
    int options = __WALL;
    pid_t tid;
...
std::vector<std::byte> sdb::process::read_memory(virt_addr address,
...
std::vector<std::byte>
sdb::process::read_memory_without_traps(virt_addr address,
                                        std::size_t amount) const {
...
int sdb::process::set_hardware_stoppoint(virt_addr address, stoppoint_mode mode,
                                         std::size_t size) {
...
int sdb::process::set_watchpoint(watchpoint::id_type id, virt_addr address,
                                 stoppoint_mode mode, std::size_t size) {
    return set_hardware_stoppoint(address, mode, size);
}

sdb::watchpoint& sdb::process::create_watchpoint(virt_addr address,
                                                 stoppoint_mode mode,
                                                 std::size_t size) {
...
bool sdb::process::should_resume_from_syscall(const stop_reason& reason) {
    /*
    No need to check syscall_catch_policy::mode::none, becaue in this case, we
    don't continue the process with PTRACE_SYSCALL so no stop would be triggered
    in the first place.
     */
...
void sdb::process::set_pc(virt_addr address, std::optional<pid_t> otid) {
    get_registers(otid).write_by_id(register_id::rip, address.addr());
}

sdb::registers& sdb::process::get_registers(std::optional<pid_t> otid) {
    auto tid = otid.value_or(current_thread_);
    return threads_.at(tid).regs;
}

const sdb::registers&
sdb::process::get_registers(std::optional<pid_t> otid) const {
...
std::optional<sdb::stop_reason> sdb::process::handle_signal(stop_reason reason,
                                                            bool is_main_stop) {
    auto tid = reason.tid;
...
sdb::registers sdb::process::inferior_call(
    ...
    rsp -= 8;
    write_memory(sdb::virt_addr{rsp}, sdb::to_byte_span(return_addr.addr()));
