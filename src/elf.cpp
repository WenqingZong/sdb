/*
Elf files can be very large, so we need to use mmap to map them into the virtual
memory of our process, pretent we've read the file completely into memory. OS
will only load necessary parts of the file when required.
*/
...
sdb::elf::get_section_contents(std::string_view name) const {
    /*
    if (auto var = init; var) syntax allows us to initialize a varialbe and test
    its value without having to declare the variable outside the scope of the if
    statement
    */
...
const Elf64_Shdr*
sdb::elf::get_section_containing_address(file_addr addr) const {
...
const Elf64_Shdr*
sdb::elf::get_section_containing_address(virt_addr addr) const {
    for (auto& section : section_headers_) {
...
std::optional<sdb::file_addr>
sdb::elf::get_section_start_address(std::string_view name) const {
    if (auto sect = get_section(name); sect) {
...
const sdb::elf*
sdb::elf_collection::get_elf_containing_address(virt_addr address) const {
    for (auto& elf : elves_) {
...
const sdb::elf*
sdb::elf_collection::get_elf_by_path(std::filesystem::path path) const {
    for (auto& elf : elves_) {
        if (elf->path() == path) {
            return elf.get();
        }
...
const sdb::elf*
sdb::elf_collection::get_elf_by_filename(std::string_view name) const {
    for (auto& elf : elves_) {
