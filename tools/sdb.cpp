...
std::string get_sigtrap_info(const sdb::process& process,
...
std::string get_signal_stop_reason(const sdb::target& target,
                                   sdb::stop_reason reason) {
...
void print_source(const std::filesystem::path& path, std::uint64_t line,
    ...
    // {} is Uniform Initialization introduced in C++ 11.
    // Sp, char c{} is equivalent to char c = 0;
    // char c just declears a char variable without initialization,
    // so its value maybe anything.
    char c{};
...
void handle_register_read(sdb::target& target,
...
void handle_register_command(sdb::target& target,
                             const std::vector<std::string>& args) {
    if (args.size() < 2) {
        print_help({"help", "register"});
        return;
        ...
        target.breakpoints().for_each([](auto& bp) {
            ...
            if (auto func_bp = dynamic_cast<sdb::function_breakpoint*>(&bp)) {
                fmt::print("function = {}", func_bp->function_name());
                           ...
                           dynamic_cast<sdb::line_breakpoint*>(&bp)) {
                fmt::print("file = {}, line = {}", line_bp->file().string(),
                           line_bp->line());
            } else if (auto addr_bp =
                           dynamic_cast<sdb::address_breakpoint*>(&bp)) {
...
void handle_breakpoint_set_command(sdb::target& target,
                                   const std::vector<std::string>& args) {
    bool hardware = false;
    if (args.size() == 4) {
        if (args[3] == "-h") {
    ...
    if (args[2].find("0x") == 0) {
        ...
        target.create_address_breakpoint(sdb::virt_addr{*address}, hardware)
            .enable();
...
void handle_breakpoint_toggle(sdb::target& target,
                              const std::vector<std::string>& args) {
    auto command = args[1];
    ...
    if (dot_pos != std::string::npos) {
        ...
        auto site_id =
            sdb::to_integral<sdb::breakpoint_site::id_type>(site_id_str);
        if (!site_id) {
...
void handle_disassemble_command(sdb::process& process,
    ...
    while (it != args.end()) {
        if (*it == "-a" and it + 1 != args.end()) {
            ...
            if (!opt_n) {
                sdb::error::send("Invalid instruction cound");
            }
...
void handle_syscall_catchpoint_command(sdb::process& process,
                                       const std::vector<std::string>& args) {
...
void handle_catchpoint_command(sdb::process& process,
                               const std::vector<std::string>& args) {
    if (args.size() < 2) {
...
void handle_thread_command(sdb::target& target,
                           const std::vector<std::string>& args) {
    if (args.size() < 2) {
    ...
    for (auto& scope : scopes) {
        for (auto& var : scope.children()) {
            ...
            if (tag == DW_TAG_variable or tag == DW_TAG_formal_parameter and
                ...
                auto str = sdb::typed_data{std::move(value), type}.visualize(
                    target.get_process());
                fmt::print("{}: {}\n", name, str);
                seen.insert(name);
            }
...
void handle_variable_read_command(sdb::target& target,
                                  const std::vector<std::string>& args) {
    auto name = args[2];
    auto pc = target.get_pc_file_address();
...
void handle_variable_location_command(sdb::target& target,
                                      const std::vector<std::string>& args) {
    auto name = args[2];
    auto pc = target.get_pc_file_address();
    ...
    auto print_simple_location = [](auto* loc) {
        if (auto reg_loc =
                std::get_if<sdb::dwarf_expression::register_result>(loc)) {
