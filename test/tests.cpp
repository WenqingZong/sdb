...
TEST_CASE("Can iterate breakpoint sites", "[breakpoint]") {
    ...
    proc->breakpoint_sites().for_each(
        /*
        Init-Capture, use mutable to change the value of addr each time.
        */
...
TEST_CASE("Breapoint on address works", "[breakpoint]") {
    bool close_on_exec = false;
    sdb::pipe channel(close_on_exec);
...
TEST_CASE("Watchpoint detects read", "[watchpoint]") {
    ...
    auto func = virt_addr(from_bytes<std::uint64_t>(channel.read().data()));
    auto& watch =
        proc->create_watchpoint(func, sdb::stoppoint_mode::read_write, 1);
    watch.enable();
...
TEST_CASE("Suscall mapping works", "[syscall]") {
...
TEST_CASE("Syscall catchpoints work", "[catchpoint]") {
    ...
    auto proc =
        process::launch("./build/test/targets/anti_debugger", true, dev_null);
...
TEST_CASE("ELF parser works", "[elf]") {
    auto path = "./build/test/targets/hello_sdb";
    sdb::elf elf(path);
...
TEST_CASE("Find main", "[dwarf]") {
    auto path = "./build/test/targets/multi_cu";
    sdb::elf elf(path);
    sdb::dwarf dwarf(elf);
...
TEST_CASE("Range list", "[dwarf]") {
    ...
    std::vector<std::uint64_t> range_data{
        0x12341234, 0x12341236, ~0ULL, 0x32, 0x12341234, 0x12341236, 0x0, 0x0,
    };

    auto bytes = reinterpret_cast<std::byte*>(range_data.data());
    sdb::range_list list(cu.get(), {bytes, bytes + range_data.size()},
                         file_addr{});
...
TEST_CASE("Line table", "[dwarf]") {
    ...
    // Here the expected line number is difference from book, because I have
...
TEST_CASE("Source-level stepping", "[target]") {
    auto dev_null = open("/dev/null", O_WRONLY);
    auto target = target::launch("./build/test/targets/step", dev_null);
    auto& proc = target->get_process();
...
TEST_CASE("Stack unwinding", "[unwind]") {
    auto target = target::launch("./build/test/targets/step");
    auto& proc = target->get_process();
...
TEST_CASE("Shared library tracing works", "[dynlib]") {
    auto dev_null = open("/dev/null", O_WRONLY);
    auto target = target::launch("./build/test/targets/marshmallow", dev_null);
    auto& proc = target->get_process();
...
TEST_CASE("Multi-threading works", "[threads]") {
    auto dev_null = open("/dev/null", O_WRONLY);
    auto target =
        target::launch("./build/test/targets/multi_threaded", dev_null);
    auto& proc = target->get_process();
...
TEST_CASE("Can read global integer variable", "[variable]") {
    auto target = target::launch("./build/test/targets/global_variable");
    auto& proc = target->get_process();
...
TEST_CASE("DWARF expressions work", "[dwarf]") {
    std::vector<std::uint8_t> piece_data = {
        DW_OP_reg16,     DW_OP_piece, 4,    DW_OP_piece, 8,
    ...
    auto expr =
        sdb::dwarf_expression(target->get_main_elf().get_dwarf(), data, false);
...
TEST_CASE("Global variables", "[variable]") {
    ...
    auto name = target->resolve_indirect_name("sy.pets[0].name",
...
TEST_CASE("Local variables", "[variable]") {
    auto dev_null = open("/dev/null", O_WRONLY);
    auto target = target::launch("./build/test/targets/blocks", dev_null);
    auto& proc = target->get_process();
...
TEST_CASE("Member pointers", "[variable]") {
    auto target = target::launch("./build/test/targets/member_pointer");
    auto& proc = target->get_process();
